"""
Run permutation+bootstrap analysis of GLMs. Fit each session X and test on another session Y.
...
STRUCTURES = [['LGd','VISp'],['VISl','VISp'],['VISl','VISp'],['LGd','VISl'],['VISp', 'VISl']]
...
def main():
            ...
            for yf in Y_FEATURE:
                ...
                for pseudo_session in SESSIONS:
                    ...
                    for loo_trial_idx in range(60): # index to drop
                        ...
                        x = df_p.loc[(df_p['session_id']==pseudo_session) & \
                                        (df['trial_idx']!=loo_trial_idx), X_FEATURE]
                        x = x.loc[:, (slice(None), xs)] 
                        x = np.concatenate((np.ones((len(x),1)),x),axis=1)
                        scaler = preprocessing.StandardScaler().fit(x)
                        x_scaled = scaler.transform(x)
    ...
    # save results
    df_results.to_csv('scratch/glm_bootstrap_v2.csv', index=False)

    # compute significance
    df_glm = pd.read_csv("/scratch/glm_results.csv")
    df_bootstrap = pd.read_csv('/scratch/glm_bootstrap_v2.csv')
    p_values = []
    ...
    df_glm.to_csv('scratch/glm_stats_v2.csv', index=False)
